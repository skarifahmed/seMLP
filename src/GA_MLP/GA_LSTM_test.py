...
# if data == "snp500": 
#     #S&P500 dataset
#     M = loadmat('S&P500.mat')
#     inputs = M['snp500_input']
#     targets = M['snp500_target']
#     # train_end = int(0.7 * len(inputs))
#     trainX = inputs[11000:13600]
#     trainY = targets[11000:13600]
#     testX = inputs[13600:15600]
#     testY = targets[13600:]
#     NUM_FEATURES = 5
# elif data == "mackey":
# # Mackey dataset
...
def layersToJson(layers, configs):
    lstm_layers = []
    for i, layer in enumerate(layers):
		...
		# 	"rate": 0.2
        # }
        lstm_layers.append(single_layer)
    ...
    lstm_layers.append(dense_layer)

    print ("Correct layers: ", layers)
    print ("LSTM Layer: ", lstm_layers)

    configs["model"]["layers"] = lstm_layers

    return configs

def train(model, orig_configs):
    ...
    for i in range(3):
        ...
        # layers
        configs = layersToJson(model_new.layers, orig_configs)
        ...
        x_test_un, y_test_un = data.get_test_data(
            seq_len=configs['data']['sequence_length'],
            normalise = False
        )

        # Random walk
        shifted_y = y_test_un[:-1]
        after_y = y_test_un[1:]
        corr_naive, p_value = pearsonr(after_y.flatten(), shifted_y.flatten())

        de_normal = data.de_normalise_windows(y_test)

        # out-of memory generative training
        steps_per_epoch = math.ceil((data.len_train - configs['data']['sequence_length']) / configs['training']['batch_size'])
        lstm.train_generator(
            data_gen_train=data.generate_train_batch(
                seq_len=configs['data']['sequence_length'],
                batch_size=configs['training']['batch_size'],
                normalise=configs['data']['normalise']
            ),
            data_gen_test=data.generate_test_batch(
                seq_len=configs['data']['sequence_length'],
                batch_size=configs['training']['batch_size'],
                normalise=configs['data']['normalise']
            ),
        ...
        predictions_point = lstm.predict_point_by_point(x_test)

        denorm_y = data.de_normalise_windows(predictions_point)
        model_new.predictions = denorm_y

        # Get Pearson's R2
        corr_relative, p_value = pearsonr(y_test.flatten(), predictions_point.flatten())
        corr_real, p_value = pearsonr(y_test_un.flatten(), denorm_y.flatten())

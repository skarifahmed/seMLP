...
def createModel():
    ...
    for k in range(num_layers): 
        ...
        while single_layer == 0:
            print ("single layer in create model == zero", delta)
            gen_delta = np.random.rand()
            while gen_delta == 0:
                print ("gen_delta in create model == zero")
                gen_delta = np.random.rand()
            delta[k] = gen_delta
            single_layer = int(delta[k] * layer)
            print ("new single layer", single_layer)
        layers.append(single_layer)
    model = multi_layer(layers, 0, delta)
...
def crossover(pop, total_pop):
    ...
    for i in range(int((0.9 * fittest_length)/2)):
        ...
        # For testing
        # c1_layers[1] = 0
        print ("Old c1_layers and c2_delta: ", c1_layers, c2_delta)
        ...
        # Check for zeroes
        while zero_flag:
            c1_bit, c2_bit, c1_layers, c2_layers,c1_delta, c2_delta, zero_flag = checkZero(c1_bit, c2_bit, c1_layers, c2_layers, c1_delta, c2_delta, dad, mum)
            # Get another set of parents
            parents_counter += 1
            # For testing
            # parents_counter = 3

            if zero_flag and parents_counter<3:
                print ("Getting another set of parents", parents_counter)
                dad, mum = getParents(total_pop, odds)
                while dad == mum:
                    dad,mum = getParents(total_pop, odds)
            if zero_flag and parents_counter>=3:
                print ("Randomly generating values now")
                # Randomly generate values
                model1, layer_c1 = createModel()
                model2, layer_c2 = createModel()
                
                #### c1 and c2 bits
                # c1
                bin_layer = format(layer_c1, BINARY_DIGITS_LIMIT)
                c1_layers = model1.layers
                c1_delta = model1.delta
                c1_bit = []
                # model.chromosome = [model.delta]
                for j in range(len(model1.layers)):
                    c1_bit.append(bin_layer)
                # c2
                bin_layer = format(layer_c2, BINARY_DIGITS_LIMIT)
                c2_layers = model2.layers
                c2_delta = model2.delta
                c2_bit = []
                # model.chromosome = [model.delta]
                for j in range(len(model2.layers)):
                    c2_bit.append(bin_layer)

                zero_flag = False
                print ("New random values: ", c1_layers, c2_delta)
...
def checkZero(c1_bit, c2_bit, c1_layers, c2_layers, c1_delta, c2_delta, dad, mum, zero_counter=0):
    ...
    while zero_counter<4:
        ...
        while z < len(c1_layers):
            ...
            if(c1_layers[z] == 0):
                ...
                print ("z: ", z, len(c1_layers))
                flag1 = True
                c1_bit, c2_bit, c1_layers, c2_layers, c1_delta, c2_delta = crossoverSingle(dad, mum)
                z=0
                zero_counter+=1
                break
        ...
        if not flag1:
            ...
            while y < len(c2_layers):
                if(c2_layers[y] == 0):
                    print ("inside zero loop of c2")
                    print ("y: ", y, len(c2_layers))
                    flag2 = True
                    c1_bit, c2_bit, c1_layers, c2_layers, c1_delta, c2_delta = crossoverSingle(dad, mum)
                    y=0
                    zero_counter+=1
    ...
    # For testing
    # zero_flag = True
    return c1_bit, c2_bit, c1_layers, c2_layers, c1_delta, c2_delta, zero_flag
...
def crossoverSingle(dad, mum):
    ...
    return c1_bit, c2_bit, c1_layers, c2_layers, c1_delta, c2_delta
...
def buildGraph(x, neurons, keep_prob):
    ...
    with tf.variable_scope("test_layer"):
        ...
        output_layer  = tf.matmul(activated, output_weights, name='output_matmul') + output_biases
...
def train(model, orig_configs, identifier, beta = BETA, batch_size = BATCH_SIZE):
#    Deconstruct the model
    neurons = model.layers

    layer_identifier = ""
    for neuron in neurons:
        layer_identifier += str(neuron) + "_"

    # Directory for saving
    folder = identifier[0] + "/" + identifier[1] + "__" + layer_identifier
    if not os.path.exists(folder): os.makedirs(folder)
    log_name = folder + "/" + "log_file"
    log_single = open(log_name,'w')
    log_single.write(str(neurons))
    ...
    for step in range(3):
        ...
        # Mean Square Error
        error = tf.sqrt(tf.reduce_mean(tf.square(y_ - output_layer)), name='test_your_error')
        ...
        with tf.Session() as sess:   #Maybe change this to outside the folds loop
            ...
            # Save model to disk
            # path = "saved_models/my-model" + str(step) + ".ckpt"
            path = folder + "/" + str(step+1) + ".ckpt"
...
def main():
    ...
    save_evol = "saved_models/" + "0"
    for k in range(len(all_models)):
        print ("---------------------------------------")
        print ("Generation:", all_models[k].generation, "Number:", k+1, "/", len(all_models))
        specific = str(all_models[k].generation) + "_" + str(k+1)
        identifier = [save_evol, specific]
        train(all_models[k], configs, identifier)
...
#  GA Evolution
    for k in range(EVOLUTIONS):
        ...
        save_evol = "saved_models/" + str(k+1)
        if not os.path.exists(save_evol): os.makedirs(save_evol)

        for j in range(start, len(pop)):
            print ("---------------------------------------")
            print ("Generation:", pop[j].generation, "Number:", j+1, "/", len(pop))
            specific = str(pop[j].generation) + "_" + str(j+1)
            identifier = [save_evol, specific]
            train(pop[j], configs, identifier)
        # Calculate Fitness
        ...
        total_pearson, new_max = calcIQR(total_pearson, 0)
        for pearson in total_pearson:
            sum_pearson += pearson
        mean_pearson.append(sum_pearson / len(total_pearson))
        print ("New pearson length in Main: ", len(total_pearson))
        # Iterate thru rms array and remove outliers too
